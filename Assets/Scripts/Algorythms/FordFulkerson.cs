...
namespace edu.ua.pavlusyk.masters
{
  ...
  public class FordFulkerson
  {
    ...
    public FordFulkerson(List<Vertex> vertices)
    {
      Nodes = new Dictionary<int, Node>();
      Edges = new Dictionary<string, Edge>();
      Vertices = vertices;
    ...
    private void GetLists(List<Vertex> vertices, out string[] nodes, out string[] edges)
    {
      ...
      for (int i = 0; i < n; i++)
      {
        nodes[i] = vertices[i].Index.ToString();
        
        for (int j = 0; j < n; j++)
        {
          edges[k] = vertices[i].Index + " " + vertices[j].Index + " " +
                     (vertices[i].ConnectedTo.ContainsKey(vertices[j]) ? 
            Graph.Oriented && vertices[i].ConnectedTo[vertices[j]] < 0 ? 
             0 : Math.Abs(vertices[i].ConnectedTo[vertices[j]]) : 0);
          k++;
        }
      }
    }
    ...
    void ParseData()
    {
      Reset();
      string[] names, edges;
      GetLists(Vertices, out names, out edges);
      /** 
    ...
    void Algo(int from, int to)
    {
      var nodeSource = Nodes[from];
      var nodeTerminal = Nodes[to];
